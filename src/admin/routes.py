        ...
        async function loadDashboard() {{
            try {{
                ...
                document.getElementById('dashboard-content').innerHTML =
                    '<p style="color: white; text-align: center;">Failed to load dashboard. Check console for details.</p>';
            }}
        }}
...
@router.get("/api/dau")
async def get_dau(session: AsyncSession = Depends(get_db)):
...
async def get_messages(days: int = 7, session: AsyncSession = Depends(get_db)):
...
async def get_top_crops(limit: int = 5, session: AsyncSession = Depends(get_db)):
...
async def get_messages_log(limit: int = 50, session: AsyncSession = Depends(get_db)):
...
async def get_broadcast_health(session: AsyncSession = Depends(get_db)):
